...
func AddTxFlagsToCmd(cmd *cobra.Command) {
	f := cmd.Flags()
	f.StringP(tmcli.OutputFlag, "o", "json", "Output format (text|json)")
	f.String(FlagKeyringDir, "", "The client Keyring directory; if omitted, the default 'home' directory will be used")
	...
	f.String(FlagFees, "", fmt.Sprintf("Fees to pay along with transaction; eg: 10uatom. Set to %q to calculate sufficient fees and gas automatically. By omitting this flag, the tx uses the required fees (but not the required gas)", FeesFlagAuto))
	...
	f.StringP(FlagBroadcastMode, "b", BroadcastSync, "Transaction broadcasting mode (sync|async|block)")
	...
	f.String(FlagKeyringBackend, DefaultKeyringBackend, "Select keyring's backend (os|file|kwallet|pass|test|memory)")
